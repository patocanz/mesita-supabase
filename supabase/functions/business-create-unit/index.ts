...
async function rankPhotosWithVision(
  ...
  try {
    ...
    if (!r.ok) {
      const errText = (await r.text()).slice(0, 240);
      console.error("[business-create-unit] vision_rank HTTP", r.status, errText);
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : "parse error";
      console.error("[business-create-unit] vision_rank parse", msg, raw.slice(0, 200));
    ...
    console.error("[business-create-unit] vision_rank exception", msg);
